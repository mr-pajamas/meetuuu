...
*/
  ...
  Meteor.sendVerificationMessage = function (mobile) {
    ...
    var messageFormat = _.template("【MeetUUU】验证码：<%= code %>。您正在验证您的手机，验证码<%= minutes %>分钟内有效");

    //console.log(mobile + ": " + messageFormat({code: code, minutes: ttlAsMinutes}));
...
if (Meteor.isClient) {
  ...
  Meteor.requestVerificationMessage = function (mobile, callback) {
    ...
    */
    Meteor.call("sendVerificationMessage", mobile, callback);
  };
