...
if (Meteor.isServer) {
  ...
  Meteor.publishComposite("groupMembers", function (groupId, selector, options) {
    ...
    selector = _.pick(selector, "nickname", "joinDate", "role");
    options.sort = _.pick(options.sort, "nickname", "joinDate", "role");

    return {
      find: function () {
        var _selector = _.extend({groupId: groupId}, selector);
        var _options = {};
      ...
      children: [
        {
          find: function (membership) {
            return Meteor.users.find(membership.userId);
          }
        }
      ]
    };
